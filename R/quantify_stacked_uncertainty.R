...
quantify_stacked_uncertainty <- function(fit,
  ...
  require(rstan)
  incl_random_efx <- ifelse2(sum(fit$super_fit$data$Zmat) == 0,
                             F,
                             T)
  ...
  if (l == "linear") {
    ...
    dispersion <- fit$super_fit$data$final_fit$tau
...
model {
  target += normal_lpdf(etahat | eta, sigmaSq_etahat);
  target += normal_lpdf(eta | mu_eta, sigma_eta);
  sigmaSq_etahat ~ inv_gamma(2,25);
  ", link_likelihood, "
}
")
    stan.data <- list(
