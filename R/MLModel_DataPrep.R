...
MLModel_DataPrep <- function(fit,name = "Default",y){
  MLModel(
    ...
    fit = function(formula, data, weights, ...) {
      ...
      fitNew <- fit(as.formula(paste0(y," ~ ",paste(colnames(dat)[!(colnames(dat) %in%
                                    c(y,'(names)'))],collapse = " + "),collapse = "")),
                    data = temp_dat,model = as.MLModel(mod))
      fit <- lm(as.numeric(dat[[y]])[1:nrow(dat)] ~ 0 + predict(fitNew,newdata = dat,type = 'prob'))
      fit$coefficients <- 1
      fit$true_fit <- fitNew
      fit$fit_mod <- mod
      return(fit)
    ...
    predict = function(object, newdata, ...) {
      ...
      if(nrow(dat) == 1){
        predict(object$true_fit,newdata = rbind(dat,dat),type = 'prob')[1]
      } else {
